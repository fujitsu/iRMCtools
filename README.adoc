...
[.lead]
*Tools and examples based on https://www.dmtf.org/standards/redfish[Redfish(R)] for Fsas Technologies iRMC <<Redfish_API>>*
...
$ irmc_cmd get Systems/0
{
  ...
  "Oem":{
    "Fsas":{
      ...
      "FirmwareInventory":{
        "@odata.id":"\/redfish\/v1\/Systems\/0\/Oem\/Fsas\/FirmwareInventory"
      },
      "MainBoard":{
...
This tool (below dir `iso`) is mentioned to mount an ISO image via NFS, CIFS, or HTTP as remote media in order to boot a server with this image. This can be used for OS installation as well as for applying Fsas Technologies Update DVD.
...
Uri='http://10.172.125.9/ISO/VMware-ESXi-6.7.0-14320388-Fsas-v480-1.iso'
----

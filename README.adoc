...
* `iRMC_CRED`: User credentials in format `user:password`  
  If this vars are not set like `export iRMC=10.11.12.13` or used like `iRMC_CRED=_admin_:___admin__ irmc_ckcon` for example then defaults from file `.irmc_env` are taken.
